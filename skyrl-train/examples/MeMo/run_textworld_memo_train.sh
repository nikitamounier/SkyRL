...
uv run --extra $INFERENCE_BACKEND -m skyrl_train.entrypoints.main_base \
  ...
  environment.skyrl_gym.textworld.memory_window=5 \
  ...
  trainer.run_name="textworld_memo_debug" \
  ...
  +modalities.memo_memory.encoder.kwargs.memo_repo_root="/home/nikitamounier/MeMo" \
  ...
  +modalities.memo_memory.trainable.projection=false \
  $@
